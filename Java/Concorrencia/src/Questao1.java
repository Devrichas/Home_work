 ...
 * Interface Runnable: Criado a classe CalculadoraLinha que implementa Runnable para definir
 * a tarefa de calcular a soma de uma linha da matriz. Isso permite que seja passada para
 * uma thread e armazena a linha especifica a ser processada.

 *  *Paralelismo: usado dentro do método main e foi feito a criação do loop for para criar 4 threads,
 * cada uma responsável por calcular a soma de uma linha da matriz. Cada thread é iniciada
 * imediatamente após sua criação, permitindo que todas as somas sejam calculadas em paralelo.

 *  *Método join(): Esta é a parte crucial que faz com que o programa espere todas as threads
 * terminarem antes de imprimirem o resultado final. Sem isso, o total poderia ser imresso antes das
 * somas estarem completas.
 */
